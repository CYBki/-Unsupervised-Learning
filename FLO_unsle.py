...
df_ = pd.read_csv("C:/Users/syorg/Desktop/flo_data_20k.csv")
df = df_.copy()
df.head()
# tarih değişkenine çevirme

# Tarih içeren sütun adlarını seç
date_columns = df.columns[df.columns.str.contains("date")]

# Seçilen tarih sütunlarını tarih formatına dönüştür
df[date_columns] = df[date_columns].apply(pd.to_datetime)

# En son sipariş tarihini bul
latest_order_date = df["last_order_date"].max()

# Analiz tarihi belirle
analysis_date = dt.datetime(2021, 6, 1)

# Her satır için 'recency' hesapla (en son alışveriş ile analiz tarihi arasındaki fark)
df["recency"] = (analysis_date - df["last_order_date"]).dt.total_seconds() / (24 * 60 * 60)

# Her satır için 'tenure' hesapla (ilk ve son alışveriş tarihleri arasındaki fark)
df["tenure"] = (df["last_order_date"] - df["first_order_date"]).dt.total_seconds() / (24 * 60 * 60)

# Analiz için kullanılacak özellikleri içeren bir DataFrame oluştur
model_df = df[["order_num_total_ever_online", "order_num_total_ever_offline",
               "customer_value_total_ever_offline", "customer_value_total_ever_online",
               "recency", "tenure"]]


# Oluşturulan DataFrame'in ilk birkaç satırını görüntüle
model_df.head()
